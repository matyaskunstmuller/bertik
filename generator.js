...
// 3. CHUNKY (H.264 MP4 BEZ ZVUKU)
// Bez zvuku a v MP4 formátu, protože VP9 WebM skládané z obrázků dělá v Chrome PIPELINE_ERROR_DECODE.
...
for (let i = 0; i < totalChunks; i++) {
    ...
    if (!fs.existsSync(chunkPath)) {
        ...
        try {
            ...
            if (count > 0) {
                // Přegenerujeme kompatibilní H.264 video.
                execSync(`ffmpeg -framerate 30 -i ${tempDir}/img_%04d.jpg -c:v libx264 -preset fast -crf 23 -pix_fmt yuv420p -an -shortest -y "${chunkPath}"`);
